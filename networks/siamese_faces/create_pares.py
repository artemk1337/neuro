...
def create(dir, fn1, fn2, n1):
    def load_img(s1, s2):
        img0 = plt.imread(f'{dir}/{s1}')
        img0 = np.array(img0, dtype='uint8')
        img1 = plt.imread(f'{dir}/{s2}')
    ...
    def same(files):
        print("<=====SAME=====>")
        if not os.path.exists(f'data/{fn1}.npy'):
            res = []
            ...
            np.save(f'data/{fn1}', res)
            # cheaker(res)
            del res

    def diff(files):
        print("<=====DIFF=====>")
        if not os.path.exists(f'data/{fn2}.npy'):
            res = []
    ...
    same(f)
    diff(f)
    a = np.load(f'data/{fn1}.npy')
    l1 = a.shape[0]
    print(a.shape)
    b = np.load(f'data/{fn2}.npy')
    print(b.shape)
    ...
    np.save(f'data/y_{n1}', y)
    del y
    np.save(f'data/x_{n1}', res)
    del res
    # cheaker(np.load(f'data/x_{n1}.npy'), np.load(f'data/y_{n1}.npy'))
...
def prepare_for_arr():
    def prepare(dir, dsave, dsave1):
        ...
        for roots, dirs, files in os.walk(dir):
            pass
        for i in range(int(len(files) * 0.4)):
            print(i)
            try:
                if not os.path.exists(f'{dsave}/{files[i]}'):
                    img = plt.imread(f'{dir}/{files[i]}')
    ...
    if not os.path.exists('data/final'):
        ...
        os.makedirs('data/final_test')
    prepare('data/male_cut', 'data/final', 'data/final_test')
    if not os.path.exists('data/final'):
        os.makedirs('data/final')
        os.makedirs('data/final_test')
    prepare('data/female_cut', 'data/final', 'data/final_test')
...
# cut_before_prepare()
prepare_for_arr()
create('data/final', 'same', 'diff', 'tr')
create('data/final_test', 'same_te', 'diff_te', 'te')
